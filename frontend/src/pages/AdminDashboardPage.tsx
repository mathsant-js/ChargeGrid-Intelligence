...
export function AdminDashboardPage() {
  ...
  return <AppShell><div className="dashboard">
    ...
    {error ? <div role="alert" className="dashboard-error"><p>{error}</p><button onClick={() => { setError(''); setAttempt(value => value + 1) }}>Tentar novamente</button></div> : !data ? <p role="status">Carregando dashboard...</p> : <>
      ...
      <section className="prediction-panel" aria-labelledby="esg-title">
        <div className="prediction-panel__heading"><div><p className="eyebrow">Fase 8 · ESG</p><h2 id="esg-title">Relatório de sustentabilidade</h2></div></div>
        <div className="prediction-grid"><div><span>Energia total</span><strong>{number(data.sustainability.energy_consumed_kwh)} kWh</strong></div><div><span>Energia solar</span><strong>{number(data.sustainability.solar_energy_kwh)} kWh</strong></div><div><span>Energia da rede</span><strong>{number(data.sustainability.grid_energy_kwh)} kWh</strong></div><div><span>Participação solar</span><strong>{number(data.sustainability.solar_percentage)}%</strong></div><div><span>CO₂ evitado</span><strong>{number(data.sustainability.avoided_co2_kg)} kg</strong></div><div><span>Fator de emissão</span><strong>{number(data.sustainability.grid_emission_factor_kg_per_kwh)} kg CO₂/kWh</strong></div><div><span>Economia estimada</span><strong>{money(data.sustainability.estimated_solar_savings)}</strong></div></div>
        <p className="advisory-note">Economia estimada teórica: valor da energia solar que teria sido comprada da rede; não representa necessariamente economia financeira real da instalação.</p>
      </section>
