...
async def list_stations(db: DbSession) -> list[ChargingStation]:
    statement = select(ChargingStation).order_by(ChargingStation.created_at, ChargingStation.id)
    return list(db.scalars(statement).all())


@router.post("", response_model=StationResponse, status_code=status.HTTP_201_CREATED)
async def create_station(payload: StationCreate, db: DbSession) -> ChargingStation:
...
async def get_station(station_id: UUID, db: DbSession) -> ChargingStation:
    return get_or_404(db, ChargingStation, station_id)


@router.patch("/{station_id}", response_model=StationResponse)
async def update_station(
    payload: StationUpdate, station_id: UUID, db: DbSession
) -> ChargingStation:
